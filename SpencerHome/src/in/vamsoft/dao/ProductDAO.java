...
public class ProductDAO {
	...
	public boolean bookproduct(String pname, int quantity, int customerid) {
		...
		if (customerDAOImpl.checkCustomerId(customerid)) {
			...
			try (PreparedStatement preparedStatement = connection
				...
				// System.out.println("check in insert");
				...
				while (rs.next()) {
					...
					try (PreparedStatement preparedStatement1 = connection
						...
						System.out.println(
								invoiceId + " " + productId + " " + pname + " " + quantity + " " + price + " " + total);
	...
	public int getLastProductId() {
		try (PreparedStatement preparedStatement = connection
				.prepareStatement("select count(*) as count from product");) {
			rs = preparedStatement.executeQuery();
			int lastCount = 0;
			while (rs.next()) {
	...
	public int getLastInvoiceId() {
		try (PreparedStatement preparedStatement = connection
				.prepareStatement("select count(*) as count from invoice");) {
			rs = preparedStatement.executeQuery();
			int lastCount = 0;
			while (rs.next()) {
